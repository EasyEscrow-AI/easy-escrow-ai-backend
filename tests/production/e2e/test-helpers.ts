 ...
 * 
 * Utility functions for PRODUCTION E2E tests
 */
...
export function loadPRODUCTIONWallets(): PRODUCTIONWallets {
  ...
  return {
    sender: loadKeypair(PRODUCTION_CONFIG.walletPaths.sender),
    receiver: loadKeypair(PRODUCTION_CONFIG.walletPaths.receiver),
    admin: loadKeypair(PRODUCTION_CONFIG.walletPaths.admin),
    feeCollector: loadKeypair(PRODUCTION_CONFIG.walletPaths.feeCollector),
  };
}
 ...
 */
export function displayWalletInfo(wallets: PRODUCTIONWallets): void {
  console.log('📋 PRODUCTION Wallet Information:');
...
export async function verifyWalletBalances(
  ...
  minSOL: number = PRODUCTION_CONFIG.testAmounts.minSOL
...
export async function waitForAgreementStatus(
  ...
  intervalMs: number = PRODUCTION_CONFIG.timeouts.polling
): Promise<any> {
...
export async function prepareDepositTransaction(
  ...
  const prepareResponse = await axios.post(
    `${PRODUCTION_CONFIG.apiBaseUrl}${endpoint}`,
    {},
    {
      headers: {
        'Content-Type': 'application/json',
