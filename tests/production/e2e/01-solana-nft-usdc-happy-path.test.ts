/**
 * PRODUCTION E2E Test - Scenario 1: Happy Path
 * 
 ...
 * 
 * Run: npm run test:PRODUCTION:e2e:happy-path
 */
...
const result = dotenv.config({ path: envPath, override: true }); // Override .env with .env.PRODUCTION
...
describe('PRODUCTION E2E - Happy Path: NFT-for-USDC Swap', function () {
  ...
  before(async function () {
    console.log('\n' + '='.repeat(80));
    console.log('🚀 PRODUCTION E2E Test - Happy Path');
    console.log('='.repeat(80));
    console.log(`   Environment: PRODUCTION`);
    console.log(`   Network: ${PRODUCTION_CONFIG.network}`);
    console.log(`   RPC: ${PRODUCTION_CONFIG.rpcUrl}`);
    console.log(`   API: ${PRODUCTION_CONFIG.apiBaseUrl}`);
    console.log(`   Program: ${PRODUCTION_CONFIG.programId}`);
    console.log('='.repeat(80) + '\n');
    ...
    // Load wallets
    console.log('📋 Loading PRODUCTION wallets...');
    ...
    console.log(`✅ Connected to Solana ${PRODUCTION_CONFIG.network}`);
  ...
  it('should setup USDC accounts for all parties', async function () {
    ...
    usdcAccounts = await setupUSDCAccounts(
      ...
      wallets.admin  // Platform/admin pays for USDC account rent
    );
  ...
  it('should create test NFT for sender', async function () {
    ...
    console.log(`   Selected NFT: ${nft.metadata.name}`);
  ...
  it('should verify receiver has sufficient USDC', async function () {
    console.log('💰 Verifying receiver USDC balance...\n');
    
    // Verify receiver has enough USDC for this test
    const hasSufficientBalance = await verifyReceiverUSDCBalance(
      connection,
      usdcAccounts.receiverAccount,
      PRODUCTION_CONFIG.testAmounts.swap
    );
    
    expect(hasSufficientBalance).to.be.true;
    console.log('   ✅ Receiver has sufficient USDC for test\n');
  });
  ...
  it('should create escrow agreement via API', async function () {
    ...
    console.log('   Request:');
    console.log(`     POST ${PRODUCTION_CONFIG.apiBaseUrl}/v1/agreements`);
    console.log(`     NFT: ${requestBody.nftMint}`);
    console.log(`     Price: ${requestBody.price} USDC`);
    console.log(`     Fee: ${PRODUCTION_CONFIG.testAmounts.fee * 100}%`);
  ...
  it('should deposit USDC into escrow', async function () {
    ...
    console.log(`   ⚠️  Receiver needs ${PRODUCTION_CONFIG.testAmounts.swap} USDC for deposit`);
    ...
    console.log(`   Requesting deposit transaction from API...`);
    const response = await axios.post(
      `${PRODUCTION_CONFIG.apiBaseUrl}/v1/agreements/${agreement.agreementId}/deposit-usdc/prepare`,
      {
  ...
  it('should verify settlement and fee distribution', async function () {
    ...
    const expectedSenderIncrease = PRODUCTION_CONFIG.testAmounts.swap * (1 - PRODUCTION_CONFIG.testAmounts.fee);
    const expectedReceiverDecrease = PRODUCTION_CONFIG.testAmounts.swap;
    const expectedFeeIncrease = PRODUCTION_CONFIG.testAmounts.swap * PRODUCTION_CONFIG.testAmounts.fee;
    ...
    if (transactions.length > 0) {
      ...
      const platformCommission = PRODUCTION_CONFIG.testAmounts.swap * PRODUCTION_CONFIG.testAmounts.fee;
      console.log(`     • EasyEscrow commission:   ${platformCommission.toFixed(6)} USDC (${(PRODUCTION_CONFIG.testAmounts.fee * 100).toFixed(1)}% of swap)`);
      console.log(`     • Seller receives:         ${(PRODUCTION_CONFIG.testAmounts.swap - platformCommission).toFixed(6)} USDC (after commission)\n`);
  ...
  it('should verify receipt generation', async function () {
    ...
    try {
      ...
      const receiptResponse = await axios.get(
        `${PRODUCTION_CONFIG.apiBaseUrl}/v1/receipts/${receiptId}`
      );
      ...
      console.log(`   🔗 Receipt URL: ${PRODUCTION_CONFIG.apiBaseUrl}/v1/receipts/${receiptId}\n`);
