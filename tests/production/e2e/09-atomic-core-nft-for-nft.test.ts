/**
 * Production E2E Test: Core NFT for SPL NFT
 * 
 * Tests the complete flow of swapping a Metaplex Core NFT for a standard SPL NFT on mainnet
 * 
...
describe('🚀 Production E2E: Core NFT ↔ SPL NFT (Mainnet)', () => {
  ...
  let takerSplNFT: { mint: PublicKey; tokenAccount: PublicKey; owner: PublicKey } | null = null;
  
  before(async function() {
    this.timeout(180000);
    
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║   PRODUCTION E2E: CORE NFT ↔ SPL NFT - MAINNET SETUP        ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    console.log('👤 Receiver (Taker - SPL NFT):', receiver.publicKey.toBase58());
    ...
    // Find SPL NFT for taker (receiver)
    console.log('\n🎨 Looking for SPL NFT in receiver wallet...');
    ...
    for (const { account, pubkey } of tokenAccounts.value) {
      ...
      if (amount === 1) {
        ...
        takerSplNFT = { mint, tokenAccount: pubkey, owner: receiver.publicKey };
        console.log(`  ✅ Found SPL NFT: ${takerSplNFT.mint.toBase58()}`);
        break;
      }
    }
    
    if (!makerCoreNFT || !takerSplNFT) {
      console.log('  ⚠️ Missing NFTs for test. Need Core NFT in sender and SPL NFT in receiver.');
      this.skip();
    }
  ...
  it('should successfully swap Core NFT for SPL NFT on mainnet', async function() {
    this.timeout(180000);
    ...
    console.log('🧪 Test: Core NFT ↔ SPL NFT swap on mainnet\n');
    
    console.log('📦 Swap Details:');
    console.log(`  Maker offers: Core NFT (${makerCoreNFT.assetId.toBase58()})`);
    console.log(`  Taker offers: SPL NFT (${takerSplNFT.mint.toBase58()})`);
    ...
    const createResponse = await apiClient.post('/api/offers', {
      ...
      requestedAssets: [{
        type: 'nft',
        mint: takerSplNFT.mint.toBase58(),
      }],
    }, {
      headers: { 'idempotency-key': `prod-core-spl-${Date.now()}` },
    });
    ...
    const acceptResponse = await apiClient.post(`/api/offers/${offer.id}/accept`, {
      ...
      headers: { 'idempotency-key': `prod-accept-core-spl-${Date.now()}` },
    });
    ...
    // Verify SPL NFT now owned by sender
    const { verifyNFTOwnership } = require('../helpers/nft-helpers');
    const splVerified = await verifyNFTOwnership(connection, takerSplNFT.mint, sender.publicKey);
    expect(splVerified).to.be.true;
    console.log('  ✅ SPL NFT transferred to sender');
    
    console.log('\n✅ Production Core NFT ↔ SPL NFT swap COMPLETE!');
  });
