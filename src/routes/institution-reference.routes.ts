...
router.get('/api/v1/institution/sanctioned-regions', standardRateLimiter, requireInstitutionAuth,
  async (_req: InstitutionAuthenticatedRequest, res: Response) => {
    try {
      res.status(200).json({ success: true, data: SANCTIONED_REGIONS, source: 'OFAC, EU, UN consolidated', timestamp: new Date().toISOString() });
    } catch (error: any) {
      res.status(500).json({ error: 'Internal Error', message: error.message, timestamp: new Date().toISOString() });
    }
  });

router.get('/api/v1/institution/corridors', standardRateLimiter, requireInstitutionAuth,
  async (_req: InstitutionAuthenticatedRequest, res: Response) => {
    try {
      const corridors = await prisma.institutionCorridor.findMany({
        where: { status: 'ACTIVE' },
        select: { id: true, code: true, sourceCountry: true, destCountry: true, minAmount: true, maxAmount: true, requiredDocuments: true, riskLevel: true, status: true },
        orderBy: { code: 'asc' },
      });
      const formatted = corridors.map((c: any) => ({ ...c, minAmount: Number(c.minAmount), maxAmount: Number(c.maxAmount) }));
      res.status(200).json({ success: true, data: formatted, count: formatted.length, timestamp: new Date().toISOString() });
    } catch (error: any) {
      res.status(500).json({ error: 'Internal Error', message: error.message, timestamp: new Date().toISOString() });
    }
  });
