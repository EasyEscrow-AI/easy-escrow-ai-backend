...
/**
 * NFT Metadata (simplified Metaplex format)
 */
...
function parseNftTokenAccountData(data: Buffer): NftTokenAccountData | null {
  try {
    if (data.length !== AccountLayout.span) {
      console.error(`[NftDepositService] Invalid account data length: ${data.length}, expected: ${AccountLayout.span}`);
      return null;
    }
...
export class NftDepositService {
   ...
   */
  public async handleNftAccountChange(
    publicKey: string,
    ...
    try {
      ...
      if (mintAddress !== expectedMint) {
        console.error(`[NftDepositService] Mint mismatch: got ${mintAddress}, expected ${expectedMint}`);
        return {
          success: false,
      ...
      try {
        ...
        console.log(`[NftDepositService] Fetched NFT metadata:`, JSON.stringify(metadata, null, 2));
      } catch (error) {
      ...
      const deposit = await prisma.deposit.create({
        data: {
          ...
          status: tokenAccountData.amount === BigInt(1) ? 'CONFIRMED' : 'PENDING',
          blockHeight: BigInt(context.slot),
          confirmedAt: tokenAccountData.amount === BigInt(1) ? new Date() : null,
          nftMetadata: metadata || { mint: mintAddress },
        },
      });
    ...
    try {
      ...
      if (metadataAccount.uri) {
        try {
          ...
          console.warn(`[NftDepositService] Failed to fetch off-chain metadata from ${metadataAccount.uri}:`, error);
          return {
            mint: mintAddress,
            onChain: metadataAccount,
          };
   ...
   */
  private async getMetaplexMetadataAccount(mintAddress: string): Promise<NftMetadata | null> {
    try {
      ...
      console.log(`[NftDepositService] Found metadata account with ${accountInfo.data.length} bytes`);
   ...
   */
  private async fetchOffChainMetadata(uri: string): Promise<OffChainMetadata> {
    try {
      ...
      const metadata = await response.json() as OffChainMetadata;
      return metadata;
    } catch (error) {
   ...
   */
  public async verifyNftDeposit(agreementId: string): Promise<{
    deposited: boolean;
    mint?: string;
    metadata?: any;
  }> {
    try {
      ...
      if (deposit) {
        return {
          ...
          metadata: deposit.nftMetadata as any,
        };
      }
   ...
   */
  public async validateNftMint(mintAddress: string): Promise<{
    valid: boolean;
    isNft: boolean;
    metadata?: any;
  }> {
    try {
      ...
      const isNft = metadata && metadata.onChain;
