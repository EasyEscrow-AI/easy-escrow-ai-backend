...
export class InstitutionReportsService {
  ...
  async getReceipts(
    ...
    if (!type || type === 'escrow') {
      ...
      for (const e of escrows) {
        results.push({
          ...
          fee: Number(e.platformFee),
          currency: 'USDC',
          corridor: e.corridor,
          status: e.status,
          payerWallet: e.payerWallet,
          recipientWallet: e.recipientWallet,
          txSignature: e.releaseTxSignature,
          completedAt: e.resolvedAt,
    ...
    return {
      ...
      total: escrowTotal + paymentTotal,
      limit,
      offset,
    };
  }
  ...
  async getEscrowLog(
    clientId: string,
    params: { escrowId?: string; from?: string; to?: string; limit?: number; offset?: number }
  ) {
...
export function getInstitutionReportsService(): InstitutionReportsService {
  if (!instance) {
    instance = new InstitutionReportsService();
  }
  return instance;
}
